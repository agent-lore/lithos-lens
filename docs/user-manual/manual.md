...
- **Dependency cycles** come first, in a warning panel. A cycle is work that blocks itself, so nothing in it can start — and everything downstream of it is marked *blocked via cycle* in the layers below. Lithos decides what is in a cycle; when the edges Lens fetched show the loop, it also shows the members and one representative path around it. When they do not, the cycle is still named, with Lithos's own explanation: under *through tasks outside this scope* if the task Lithos names as the blocker is outside this graph, and under *shape unavailable* otherwise — which says only that these edges do not show the loop, not where it runs.
