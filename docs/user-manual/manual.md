...
Lens is honest about what it could not read. If a task's edges could not be fetched it is marked **edges unknown** and is never quietly filed as "isolated"; if a blocker's status could not be read, its dependent is marked **blocked by unresolvable predecessor**; and when either happens, the longest chain is written as "≥ N" with a note, because the real chain can only be longer. If the read behind the cycle panel was cut short, failed, or was never made — a task with no project for Lens to read by, or a page that ran out of time before that read could be sent — a banner says so, saying which of those it was, and the affected tasks read **cycle status unknown** — never a silent "no cycle". And if a scope names more projects than Lens will check in one go, the page says so up front rather than checking some of them and drawing the rest as if they were fine.
