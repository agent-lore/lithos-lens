...
- **Dependency cycles** come first, in a warning panel. A cycle is work that blocks itself, so nothing in it can start — and everything downstream of it is marked *blocked via cycle* in the layers below. Lithos decides what is in a cycle; Lens shows the members and one representative path around the loop. A cycle that runs through tasks outside this scope is listed separately, under *through tasks outside this scope*, with Lithos's own explanation.
- **The legend** spells out what each kind of line means in words ("A → B means A blocks B"), plus the two conventions the page uses: a *ghost* is a task outside this scope, and a bracketed group is a cycle.
