...
- **Layers** are the graph itself, arranged by **dependency depth**: layer 0 is work that nothing else in this graph blocks — it waits on nothing here — and a task two layers down has a chain of two dependencies behind it in the picture. That is planned order, not a start signal — the layers are drawn over every dependency edge the page fetched, satisfied ones included, so a layer does not have to finish before the next one begins. Whether one task can start is its own question: read the **blocked by** lines under its row, the *ready* / *blocked* sections on the [dashboard](#the-tasks-dashboard), or the task's own detail page. Every row carries the task's status, its type, who has claimed it, and — under it — what it is blocked by. A row for a task outside this scope is dimmed, carries its own project chip, and offers a link to *that* project's graph, so crossing a project boundary is something you choose rather than something that happens to you.
