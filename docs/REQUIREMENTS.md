...
- **Active dependency projection:** when resolved tasks are in scope, their satisfied edges are drawn as history (faded; labelled `satisfied` in text) but every claim about *blocking* — the longest blocking chain, focus lighting, downstream impact — is computed over the **active** projection only: a dependency edge is active when its predecessor is `open` or `cancelled` (cancelled blocks forever; completed blocks nothing). Layers and isolation use all fetched dependency edges, because they describe planned sequence and structure, not what blocks now.
...
- **Dependency edges have three states — and only dependency edges are ever "satisfied":** a `blocks` / `waits_on_gate` edge is `active` (predecessor `open` or `cancelled`), `satisfied` (predecessor `completed`), or `unknown` (the predecessor is a ghost whose `task_get` failed). Completion resolves a dependency; it does not erase hierarchy or provenance. **Satisfied edges are dropped, not ghosted:** when a completed predecessor is out of scope its dependency edge goes (it returns, faded, under `include_resolved`). **Unknown edges are visible, excluded from the active computation, and degrade every claim they touch** (chain, focus lit-set, impact) to a lower bound — treating them as active would make the result no bound at all, omitting them would hide a possibly-live relationship. **`parent_child` and `discovered_from` are never dropped:** a far endpoint outside the node set is resolved as a faded, leaf-only *context ghost* (counted toward `max_tasks`) — parents always (the text hierarchy tree is always rendered; the hierarchy is a single-parent forest), provenance sources only when the provenance overlay is enabled — so an open child keeps its completed parent and an open follow-on keeps its provenance.
...
- **Isolated tasks:** a task with no fetched `blocks`/`waits_on_gate` edge in the scope (active or satisfied) is *isolated* — never a task whose edge read failed; rendered in an "N isolated tasks" disclosure — collapsed by default on a project scope, open by default on an epic scope (hiding an epic's edge-less children would misrepresent its progress) — with a toggle (`isolated=1|0`). Isolated tasks still count toward `max_tasks`.
