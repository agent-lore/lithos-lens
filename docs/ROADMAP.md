...
| 7 | Project convention split: `metadata.project` (filters, spawn inheritance) vs `project:` tags (existing corpus) — both in active use with disagreeing counts | Pick one canonical convention | Lens honors both unconditionally — the `project_convention` posture knob is retired (REQUIREMENTS §4.4) — and warns on disagreement. |
