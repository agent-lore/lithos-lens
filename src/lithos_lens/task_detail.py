...
class TaskDetailData:
    ...
    # The WHOLE findings timeline, oldest-first. Unlike every other list on
    # this page it is not paged: §5.6 says "no paging controls", and the
    # per-row cost that justifies paging elsewhere is not here — one
    # ``lithos_finding_list`` returns every summary in one round trip. What IS
    # bounded is the note-title fan-out (see :func:`resolve_finding_notes`).
...
async def load_findings_timeline(
    ...
    ``findings.html`` reads only ``findings`` and ``findings_state``, so
    running the full :func:`load_task_detail` for it would buy the whole
    graph fan-out — the edge list, the children, four pages
    ...
    views = await resolve_finding_notes(lithos, findings, deadline=deadline)
    return TaskDetailData(
        task=None,
        findings=views,
        reopen_report=latest_reopen_report(findings),
...
async def resolve_finding_notes(
    ...
    *,
    ...
    """The WHOLE timeline, with knowledge-link titles for its newest page.

    Every finding renders. §5.6 is explicit — "no paging controls; very long
    timelines MAY collapse older findings behind a Show older findings
    disclosure" — and a tail that states a count without a way to reach what it
    counts is neither of those things: it makes the older history unreachable.
    The blocker mechanism does not belong here.

    What IS bounded is the only per-row cost. The findings count is
    agent-controlled in exactly the way the edge count is —
    ``lithos_finding_post`` takes ``{task_id, agent, summary, knowledge_id}``
    with no credential and no per-task cap, and ``lithos_finding_list`` takes
    no limit — but the summaries all arrive in ONE round trip, while each
    DISTINCT ``knowledge_id`` costs a ``lithos_read`` on the shared MCP
    session. So the fan-out is capped at :data:`DETAIL_PAGE_SIZE` findings'
    worth of titles (the NEWEST, which are the ones being read), and a row
    past that renders its summary with the plain "View document" label §5.6
    already specifies. No warning on those: not looking a title up is not a
    failure to look it up, and the row is not degraded — its text is all
    there.

    Two further economies on the lookups: ids are de-duplicated across the
    titled page (one read serves every finding citing the same document), and
    the read is ``max_length=1`` — frontmatter comes back complete (§6.3), so a
    ...
    timeline = sorted(findings, key=lambda item: item.created_at)
    # The bound is on the LOOKUPS, not on the list: the newest page is the one
    # whose titles get resolved, and `last_page` is the same single pagination
    # decision the related-task lists reach through.
    titled, _older = last_page(timeline)
    lookup_ids = {finding.knowledge_id for finding in titled if finding.knowledge_id}
    # dict.fromkeys: de-duplicate the cited documents, keep first-cited order.
    cited = tuple(dict.fromkeys(f.knowledge_id for f in titled if f.knowledge_id))
    ...
    for finding in timeline:
        if not finding.knowledge_id or finding.knowledge_id not in lookup_ids:
            # Either no document to link, or one this render did not look up.
            # Both render the plain label; neither is an error.
