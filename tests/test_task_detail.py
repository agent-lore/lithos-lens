...
def test_the_whole_timeline_renders_and_only_the_lookups_are_bounded(
    lithos_lens_config_env: Path,
) -> None:
    """§5.6: "no paging controls". EVERY finding renders, however many there are.

    The related-task lists are paged because each ROW costs a round trip. A
    findings timeline does not: one ``lithos_finding_list`` returns every
    summary. The only per-row cost is the note title, so that is what the page
    size bounds — the newest page gets its titles looked up, and an older row
    renders its summary with the plain "View document" label §5.6 already
    specifies for a title that is not in hand.

    Paging the timeline itself made older findings UNREACHABLE — no
    disclosure, no route, no continuation — which is both a regression on the
    unpaged timeline this replaced and the opposite of what §5.6 permits.
    """
    fake = NoteProbeClient()
    older = 7
    total = DETAIL_PAGE_SIZE + older
    _stage_findings(fake, "open-unclaimed", total)
    ...
    assert response.status_code == 200
    text = response.text
    # Every finding is rendered, oldest included.
    assert text.count("<p>Finding ") == total
    assert "Finding 0<" in text
    assert f"Finding {total - 1}" in text
    # ... but only the newest page's titles were fetched.
    assert len(fake.note_calls) == DETAIL_PAGE_SIZE
    # No tail, because nothing is being withheld.
    assert 'data-overflow-tail="findings"' not in text
    # An untitled row is not a failed one: it must carry no warning.
    assert "Could not resolve document title" not in text
...
def test_an_old_reopen_is_visible_as_a_row_and_as_the_marker(
    lithos_lens_config_env: Path,
) -> None:
    """The marker is derived from the whole findings list, and so is the list.

    A reopen older than a page of findings is exactly the history an operator
    goes to the timeline to find. It drives the header marker (which costs no
    round trip, so it never depended on paging), and — now that the timeline
    is not paged — the finding itself is still there to read.
    """
    fake = NoteProbeClient()
    ...
    fake.findings["open-unclaimed"].insert(
        ...
        FindingRecord(
            ...
            # Older than every finding whose title gets resolved.
