...
Unmetered is not unbounded. The stream has its own ceiling, counting the thing
a parked browser actually consumes, and it answers with the same 503 — so the
last test here is the other end of the exemption above.
"""
...
async def _stream_frames(app: FastAPI, path: str, count: int) -> list[bytes]:
    ...
    """
    frames: list[bytes] = []
    scope: dict[str, Any] = _http_scope(path)
...
async def _status_and_body(app: FastAPI, path: str) -> tuple[int, bytes]:
    """The status and first body frame for ``path``, without reading to the end.

    Driven with a deadline for the same reason ``_stream_frames`` is, plus one
    of its own: what this refuses is a STREAM. A test client would read the
    response to completion, so the day the refusal stops happening it would
    block on a response that never ends — and the regression would surface as a
    hung CI job rather than a failing test.
    """
    status = 0
    body = b""
...
@pytest.mark.anyio
async def test_the_event_stream_refuses_a_subscriber_past_the_hub_ceiling(
    lithos_lens_config_env: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The other end of the exemption: unmetered, but not unbounded.

    Exempting the stream from the render budget was right — a parked browser is
    not a render — but it left the count of parked browsers up to whoever can
    reach the port, each one pinning a queue and making every publish one step
    slower. So the hub keeps its own ceiling and the route turns it into the
    same 503, which EventSource surfaces as an error and tasks.js answers by
    polling: a slower board for the refused tab, rather than a process pushed
    further past a bound it has already reached.

    A ceiling of zero saturates the hub by construction, the way a render cap
    of zero does above. The asset still serves, because this bound is scoped to
    the resource it counts and nothing else.
    """
    monkeypatch.setattr(events, "MAX_EVENT_SUBSCRIBERS", 0)
    app = create_app(load_config(lithos_lens_config_env))

    status, body = await _status_and_body(app, "/tasks/events")

    assert status == 503
    assert b"poll" in body.lower()
    with TestClient(app) as client:
        assert client.get("/static/lens.css").status_code == 200
